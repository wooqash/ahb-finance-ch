                ...
                {buttons.map((button, index) => {
                    return (
                        <React.Fragment key={button.label}>
                            <a  href={button.url} target={button.openInNewTab ? "_blank" : "_self" } rel={ button.openInNewTab ? "noopener noreferrer" : ''} className={`${styles.button} ${styles['button--contained']} w-11/12 max-w-sm sm:w-7/12`}>{button.label}</a>
